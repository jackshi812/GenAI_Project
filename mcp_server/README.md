...
allowlist. The implementation never scrapes retailer pages; it calls Serper's
API only. A process-local TTL cache defaults to 180 seconds, outbound live
calls are spaced by one second, and a 50-call process cap protects API quota.
